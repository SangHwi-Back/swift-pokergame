...
final class ShuffleAlgorithm<T: Array<AnyObject>.Element> {
    ...
    func fisherYatesAlgorithm(at array: [T]) -> [T] { // Generic을 사용하여 어떠한 Array에도 적용되도록 하였습니다.
        ...
        while let index = randomIndex(deckForShuffle) {
            result.append( deckForShuffle.remove(at: index) )
        }
    ...
    func knuthAlgorithm(at array: [T]) -> [T] {
...
//            cards.swapAt(i, randomIndex)
    ...
    func ordinaryCardShuffle(at array: [T]) -> [T] {
